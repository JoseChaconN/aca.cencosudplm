<x-layout>
	...
	<div class="col-lg-12">
		<div class="card shadow ">
	        ...
	        <form method="POST" enctype="multipart/form-data" action="{{ (!empty($auditoria->id)) ? route('auditorias.update',$auditoria->id) : route('auditorias.store') }}">
	        	@csrf
	        	@if(!empty($auditoria->id))
	        		@method('PATCH')
	        	@endif
	        	<div class="card-body border-left-primary">
	        		...
	        		<div class="row">
						...
						<div class="col-md-12">
							<div class="form-group row">
								<label class="col-sm-4 col-form-label font-weight-bold">Fecha Auditoría:</label>
								<div class="col-sm-8">
									{{ (!empty($auditoria->id)) ? date('d-m-Y',strtotime($auditoria->fecha_auditoria)) : date('d-m-Y') }}									
								</div>
							</div>
						</div>
						<div class="col-md-12">
							<div class="form-group row">
								...
								<div class="col-sm-8">
									<input type="date" name="fecha_ejecucion" class="form-control" value="{{$auditoria->fecha_ejecucion}}">
								</div>
							</div>
						</div>
                        <div class="col-md-12">
								...
								<div class="col-sm-8">
									<select name="area" class="form-control">
										...
										@foreach ($area_auditoria_visita as $key => $value)
											<option {{ (old('area' , $auditoria->area) == $value['val']) ? 'selected' : ''}} value="{{$value['val']}}">{{$value['text']}}</option>
										@endforeach
									</select>
								</div>
							</div>
								...
								<div class="col-sm-8">
									<select name="programa" class="form-control">
										...
										@foreach ($programa_auditoria_visita as $key => $value)
											<option {{ (old('area' , $auditoria->programa) == $value['val']) ? 'selected' : ''}} value="{{$value['val']}}">{{$value['text']}}</option>
										@endforeach
									</select>
								</div>
							</div>
								...
								<div class="col-sm-8">
									<select name="id_seccion" class="form-control">
										...
										@foreach ($secciones as $seccion)
											<option {{ (old('area' , $auditoria->id_seccion) == $seccion->codigo) ? 'selected' : ''}} value="{{$seccion->codigo}}">{{$seccion->nombre}}</option>
										@endforeach
									</select>
								</div>
							</div>
								...
								<div class="col-sm-8">
									<select name="organismo_auditor" class="form-control">
										...
										@foreach ($organismos_auditores as $organismo)
											<option {{ (old('organismo_auditor' , $auditoria->organismo_auditor) == $organismo->id) ? 'selected' : ''}} value="{{$organismo->id}}">{{$organismo->nombre}}</option>
										@endforeach
									</select>
								</div>
							</div>
								...
								<div class="col-sm-8">
									<select name="id_planta" class="form-control">
										...
										@foreach ($proveedor->plantas as $planta)
											<option {{ (old('area' , $auditoria->id_planta) == $planta->id) ? 'selected' : ''}} value="{{$planta->id}}">{{$planta->nombre}}</option>
										@endforeach
									</select>
								</div>
							</div>
						...
						<div class="col-md-12">
							<div class="form-group row">
								...
								<div class="col-sm-8">
									<input type="text" name="porcentaje" class="form-control inputDecimal" placeholder="% Porcentaje" value="{{$auditoria->porcentaje}}">
								</div>
							</div>
						</div>
						<div class="col-md-12">
							<div class="form-group row">
								...
								<div class="col-sm-8">
									<textarea name="observaciones" id="observaciones" rows="5" class="form-control" style="resize: none;" placeholder="Observaciones">{{$auditoria->observaciones}}</textarea>
								</div>
							</div>
						</div>
						<div class="col-md-12">
							<div class="form-group row">
								...
								<div class="col-sm-8">
									<textarea name="conclusiones" id="conclusiones" rows="5" class="form-control" style="resize: none;" placeholder="Conclusiones">{{$auditoria->conclusiones}}</textarea>
								</div>
							</div>
						</div>
						<div class="col-md-12">
							<div class="form-group row">
								...
								<div class="col-sm-8">
									<textarea name="linea_proceso" id="linea_proceso" rows="5" class="form-control" style="resize: none;" placeholder="Línea de Proceso">{{$auditoria->linea_proceso}}</textarea>
								</div>
							</div>
						</div>
	        		</div>
							...
							@foreach ($imagenes_auditoria as $key => $value)
								<div class="col-md-4 mb-4">
									<div class="row">
										...
										<div class="col-md-12">
											<button class="btn btn-danger btn-sm" type="button" onclick="fnDeleteMediaFile('{{ route('biblioteca.delete.media') }}',{{ $value['id'] }},null,{{ $auditoria->id }},'auditoria')">Eliminar Imagen</button>
										</div>
									</div>
								</div>
							@endforeach
					...
					<div class="col-md-12 mt-4">
						<div class="row">
							@foreach ($auditoria_auditorias as $key => $value)
								<div class="col-md-12">
									<div class="col-md-12">
										<div class="form-group row">
									...
									<div class="col-md-12">
										<div class="form-group row">
											<div class="col-md-12">
												<a class="btn btn-primary btn-sm" download="" href="{{$adjunto_auditoria_auditorias[$key]['url']}}" target="_blank">
													Descargar Adjunto
												</a>
												<button class="btn btn-danger btn-sm" type="button" onclick="fnDeleteBibliotecaFile_2('{{ route('biblioteca.delete') }}',{{ $value['id'] }},'','')">Eliminar Adjunto</button>
											</div>
										</div>
									</div>
									<div class="col-md-12">
					...
					<div class="col-md-12 mt-4">
						<div class="row">
							@foreach ($auditoria_documentos as $key => $value)
								<div class="col-md-12">
									<div class="col-md-12">
										<div class="form-group row">
											<label class="font-weight-bold">Titulo Documento:</label> {{$value['nombre_documento']}}
									...
									<div class="col-md-12">
										<div class="form-group row">
											<div class="col-md-12">
												<a class="btn btn-primary btn-sm" download="" href="{{$adjunto_auditoria_documentos[$key]['url']}}" target="_blank">
													Descargar Adjunto
												</a>
												<button class="btn btn-danger btn-sm" type="button" onclick="fnDeleteBibliotecaFile_2('{{ route('biblioteca.delete') }}',{{ $value['id'] }},'','')">Eliminar Adjunto</button>
											</div>
								...
								<div class="col-sm-8">
									<select class="form-control id_certificacion">
										...
										@foreach ($certficaciones_vencimiento as $certificacion)
											<option {{ (old('area' , $auditoria->id_planta) == $certificacion->id) ? 'selected' : ''}} value="{{$certificacion->id}}">{{$certificacion->nombre}}</option>
										@endforeach
									</select>
								</div>
							</div>
									...
									<div class="col-md-12">
										<div class="form-group row">
											<div class="col-md-12">
												...
												<button class="btn btn-danger btn-sm" type="button" onclick="fnDeleteBibliotecaFile_2('{{ route('biblioteca.delete') }}',{{ $value['data']['id'] }},'','')">Eliminar Adjunto</button>
											</div>
										</div>
									</div>
									<div class="col-md-12">
